...
void InitVariables(void)
{
    ...
    {
        cache_item_2[i].reset(); // [31]:valid,[30]:hit,[29]:dirty,[28]-[0]:data

        if(t_replace == LRU)
        {
            LRU_priority_2[i] = 0; //For LRU policy's priority
        }
		else if(t_replace == FIFO)
		{
			FIFO_priority_2[i] = 0;
    ...
    t_write = write_back; //write policy,default write_back
#endif // SetAssociative_Random_WriteBack
    bit_block = 0, bit_block_2 = 0; //How many bits of the block.
    bit_line = 0, bit_line_2 = 0; //How many bits of the line.
    bit_tag = 0, bit_tag_2 = 0; //How many bits of the tag.
    bit_set = 0, bit_set_2 = 0; //How many bits of the set.
